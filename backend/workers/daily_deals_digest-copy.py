...
def get_active_subscriptions():
    ...
    Table used by existing keyword-alert system.
    """
    res = (
...
def get_recent_deals(hours: int):
    ...
    # IMPORTANT: keep selecting only columns you already have
    res = (
...
def build_daily_email_html(email: str, keywords: list[str], deals: list[dict]) -> str:
    # Language: Vietnamese (per your request)
    ...
    intro = f"""
      <div style="color:#444;margin:0 0 12px 0">
        ...
        Nếu có deal <b>match keyword</b>, hệ thống sẽ <b>gửi email ngay</b>.
        Nếu chưa match, bạn cứ chờ — khi có match sẽ báo liền ✅
      </div>
      <div style="margin:0 0 14px 0">
        Hôm nay chúng tôi đang có <b>{len(deals)}</b> deal mới (trong {LOOKBACK_HOURS} giờ gần nhất). Mời bạn ghé xem:
      </div>
    """
    ...
    footer = f"""
      ...
      <div style="color:#888;font-size:12px">
        Bạn nhận email này vì đã subscribe tại {SITE_BASE}.<br/>
        Nếu bạn muốn dừng nhận email, bạn có thể unsubscribe trong email xác nhận keyword alert.
      </div>
    """

    return f"""
      <div style="font-family:Arial,Helvetica,sans-serif;line-height:1.45">
        <h2 style="margin:0 0 10px 0">🛍️ BestDeals - Daily New Deals</h2>
